...
function renderTables() {
  ...
  if (productsData.length === 0) {
    ...
    ptbody.innerHTML = productsData.map(p => `
      <tr>
        <td data-label="ID" style="color:var(--text-muted);font-size:12px;">${p.ProductID || '-'}</td>
        <td data-label="Brand" style="font-weight:700;color:var(--primary);">${p.BrandName || ''}</td>
        <td data-label="Product">${p.ProductName || ''}</td>
        <td data-label="Packaging"><span class="info-badge" style="display:inline-flex;">${p.PackagingSize || ''}</span></td>
        <td data-label="Price" style="font-weight:600;">₹${parseFloat(p.UnitPrice || 0).toLocaleString('en-IN', {minimumFractionDigits:2})}</td>
  ...
  if (clientsData.length === 0) {
    ...
    ctbody.innerHTML = clientsData.map(c => {
      ...
      return `
      <tr>
        <td data-label="ID" style="color:var(--text-muted);font-size:12px;">${c.ClientID || '-'}</td>
        <td data-label="Name" style="font-weight:700;color:var(--primary);">${c.ClientName || ''}</td>
        <td data-label="Phone">${c.Phone || ''}</td>
        <td data-label="GSTIN"><span style="font-family:monospace;letter-spacing:0.5px;font-size:13px;">${c.GSTIN || ''}</span></td>
        <td data-label="Due Amount" style="text-align:right;">${dueDisplay}</td>
        <td data-label="Actions" style="text-align:right;white-space:nowrap;">
